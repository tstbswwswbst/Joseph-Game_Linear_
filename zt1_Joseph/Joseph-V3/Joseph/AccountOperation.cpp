...
int AccountOperation::SigninAccount() {  //登陆账户
    ...
    for (auto& account : aSystem) { // 遍历查找是否当前登录ID
        if (account.ID == ID) {
...
void AccountOperation::BrowseSystem () { // 浏览账户系统
    if (!p_aSystem->getAccounts().size()) {
        ...
        int cellWidth = 100;
        int cellHeight = 25;
        int numRows = static_cast<int>(p_aSystem->getAccounts().size()) + 1; // 包括表头所以加1
        ...
        for (const auto& account : sortedAccounts) {
            int rowY = startY + (i + 1) * cellHeight;
            // 输出排名
...
void AccountOperation::SearchSystem () {
    ...
    if (foundAccounts.empty()) {
        ...
        int cellWidth = 100;
        int cellHeight = 25;
        int numRows = static_cast<int>(foundAccounts.size()) + 1;
...
int AccountOperation::ExportData () {     // 导出账户信息
    char str1[20] = { '\0' };
...
int AccountOperation::ImportData () {   // 导入账户信息
    char str1[20] = { '\0' };
    InputBox(str1, 20, "** Import Accounts **\nEnter filename to import data:\nOriginal Source you can choose: accounts.txt");
    string filename(str1);
    filename.append(".txt");
